...
/* ===== template and filter functions ===== */
	...
	*/
	function shortcode($atts) {
		...
		$subscribe_button_value = apply_filters('s2_subscribe_button', __('Отправить', 'subscribe2'));
		...
		if ( 'true' == strtolower($args['nojs']) ) {
			$this->form = "<form method=\"post\"" . $action . "><input type=\"hidden\" name=\"ip\" value=\"" . $_SERVER['REMOTE_ADDR'] . "\" />" . $antispam_text . "<input placeholder=\"e-mail\" type=\"text\" name=\"email\" id=\"s2email\" value=\"" . $value . "\" size=\"" . $args['size'] . "\" />" . $wrap_text . $this->input_form_action . "</form>";
		} else {
			$this->form = "<form method=\"post\"" . $action . "><input type=\"hidden\" name=\"ip\" value=\"" . $_SERVER['REMOTE_ADDR'] . "\" />" . $antispam_text . "<input type=\"text\" name=\"email\" id=\"s2email\" placeholder=\"e-mail\" value=\"" . $value . "\" size=\"" . $args['size'] . "\" onfocus=\"if (this.value == '" . $value . "') {this.value = '';}\" onblur=\"if (this.value == '') {this.value = '" . $value . "';}\" />" . $wrap_text . $this->input_form_action . "</form>\r\n";
		}
